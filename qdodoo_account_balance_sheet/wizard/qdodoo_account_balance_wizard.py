...
class qdodoo_account_balance_search(models.Model):
    """
    科目余额表wizard视图
    """
    ...
    @api.multi
    def btn_search(self):
        sql_del1 = """delete from qdodoo_balance_partner_line where 1=1"""
        self.env.cr.execute(sql_del1)
        sql_del2 = """delete from qdodoo_account_balance_partner where 1=1"""
        self.env.cr.execute(sql_del2)
        sql_del3 = """delete from qdodoo_account_balance where 1=1"""
        ...
        for key_account in list(set(starting_balance_dict.keys() + debit_dict.keys() + credit_dict.keys())):
            ...
            sql = """
                insert into qdodoo_account_balance (period_id,starting_balance,ending_balance,account_id,company_id,debit,credit) VALUES (%s,%s,%s,%s,%s,%s,%s) returning id
                """ % (
        ...
        for key_partner in list(set(partner_key_list + account_list)):
            ...
            if key_partner[0]:
                sql = """
                insert into qdodoo_account_balance_partner (account_periodly_id,partner_id,period_id,account_id,company_id,debit,credit,start_balance,end_balance) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s) returning id
                """ % tuple(values)
            else:
                del values[1]
                sql = """
                insert into qdodoo_account_balance_partner (account_periodly_id,period_id,account_id,company_id,debit,credit,start_balance,end_balance) VALUES (%s,%s,%s,%s,%s,%s,%s,%s) returning id
                """ % tuple(values)
            self.env.cr.execute(sql)
            ...
            if move_lines:
                for move_line in move_lines:
                    ...
                    sql_line = """
                    insert into qdodoo_balance_partner_line (move_name,line_name,report_id,period_id,company_id,account_id,debit,credit) VALUES ('%s','%s',%s,%s,%s,%s,%s,%s)
                    """ % tuple(values)
